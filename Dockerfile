...
# Product: Zaruman
# Context: src-gen/Zaruman/ (same level as docker-compose.yml)
...
RUN mv spleproyekassoc.product.zaruman/*.jar ./jars/ 2>/dev/null || true && \
...
# Run the application
CMD ["java", "-cp", "jars/*:.", "spleproyekassoc.product.zaruman.Zaruman"]

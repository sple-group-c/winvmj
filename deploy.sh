...
ssh -i "$private_key_amanah" "$username_amanah@localhost" -p "$local_tunnel_port" "cd /home/prices-deployment/nix-environment && nix-shell --run 'bash prices_product_deployment.sh $product_name $product_remote_directory spleproyekassoc'"
if [ $? -eq 0 ]; then
  echo
